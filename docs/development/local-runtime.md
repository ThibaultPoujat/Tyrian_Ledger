...
No ArenaNet API key or existing database is required to start the local host.
The host initializes its local SQLite schema automatically; when personal
account features are introduced, configure an optional dedicated key through
[Local ArenaNet API Key Setup](local-secrets.md). The browser never stores or
submits it.
