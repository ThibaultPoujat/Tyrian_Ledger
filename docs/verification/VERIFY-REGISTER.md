...
| VERIFY-014 | IN PROGRESS | M10 scheduled capture and static deployment evidence is pending: record trusted workflow evidence for the 15-minute capture, configured request/concurrency/burst limits, selector validation and `develop` fallback, static-artifact contents, and the pre-publication audit before the owner makes the repository or Pages site public. | TKT-M10-06 (2026-09-02) recorded the 8.30.1 full-history audit, local validation matrix, public Pages configuration, and successful push deployment [33635230439](https://github.com/ThibaultPoujat/Tyrian_Ledger/actions/runs/33635230439). Its `selection: null` resolved the trusted `develop` fallback to `0f093969b1e63e77df933c9fe482f20e8163ed04`; its complete snapshot logged the `2/2/20` capture policy and deployed the audited static artifact to `https://thibaultpoujat.github.io/Tyrian_Ledger/`. A clean-browser review verified fresh, loading, delayed, and malformed public-site states, keyboard navigation, WCAG 2.2 AA automated checks, and no browser `/api` or Guild Wars 2 traffic. An owner-authorized rerun of 33635230439 completed 2026-09-02 14:09 UTC and restored a snapshot generated at `2026-09-02T14:08:06.6071078Z`; it is recovery evidence, not a scheduled event. After GitHub created no scheduled events, TKT-M10-06 added an offset `7,22,37,52` UTC schedule and `workflow_dispatch` recovery path; a later successful scheduled run on that hardened configuration remains required before resolution. A manual recovery run does not replace this evidence. This is deployment evidence, not confirmation of external Guild Wars 2 API behaviour; batching, schema, rate-limit, retry, and fee facts remain under VERIFY-004, -005, -006, -010, -011, and -013. | TKT-M10-06 | 2026-09-01 | — |
