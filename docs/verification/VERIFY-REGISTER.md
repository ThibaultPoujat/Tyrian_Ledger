...
| VERIFY-004 | OPEN | Exact hard limit of the `ids` batch parameter (community best-practices page documents 200 IDs per request) and exact 206 / paging behavior (max `page_size`, per-endpoint applicability). M13 owns authenticated endpoint/paging applicability; M18 owns collector-scale public batching behavior; M21 owns recipe batching applicability. | `docs/architecture/gw2-endpoint-matrix.md` — global rules; wiki `API:Best_practices` and `API:2` fetched 2026-08-21. TKT-M2-01's 2026-08-26 no-key probe observed that unparameterized `/commerce/prices?v=latest` returns a numeric ID index (27,987 entries); it did not exercise batches or 206 behavior. TKT-M9-02 (2026-08-31) uses deterministic batches of at most 200 and rejects HTTP 206 / response-ID mismatches as incomplete input. TKT-M13-03 (2026-09-06) rechecked the documented zero-based `page` and paging-header contract for authenticated transactions; it deliberately sends no `page_size`, as endpoint-specific maximum/206 behavior remains unverified. TKT-M18-02 (2026-09-08) reuses that typed 200-ID batching and rejects incomplete collector responses; no additional live batch probe was performed. TKT-M21-01 re-fetched the public `API:2/recipes` source on 2026-09-16: it confirms multi-ID object responses but no hard limit or 206 contract, so the new recipe gateway keeps 200-ID batches and rejects incomplete ID sets. TKT-M22-01 (2026-09-25) records sanitized batch/request/response discrepancy diagnostics for bounded listings but keeps every omitted listing ID unclassified and rejects the whole response: the public contract does not yet distinguish an absent listing or nontradable item from a truncated successful response. | TKT-M13-03 / TKT-M18-02 / TKT-M21-01 / TKT-M22-01 | 2026-08-21 | — |
| VERIFY-005 | OPEN | Schema version to pin per endpoint via `?v=` / `X-Schema-Version`; the latest relevant version per endpoint must be confirmed against `/v2.json?v=latest` (matrix recipes row provisionally uses schema `2022-03-09T02:00:00.000Z`). M13 owns personal TP endpoints; M19 owns account-wallet use; M21 owns recipe definition and account-recipe endpoints. | `docs/architecture/gw2-endpoint-matrix.md`; wiki `API:2` fetched 2026-08-21. TKT-M2-01's 2026-08-26 no-key `/v2.json?v=latest` probe established `2025-08-29T01:00:00.000Z` as the current global version. TKT-M9-02 rechecked that same newest global entry on 2026-08-31 and pins it for public prices, listings, and items. TKT-M13-03 (2026-09-06) retains that global pin for account and personal transaction reads after rechecking the documented schema mechanism, but has no endpoint-specific latest-schema evidence. TKT-M18-02 (2026-09-08) reuses the provisional public-price/listing pin. TKT-M19-04 (2026-09-09) rechecked the keyless route index and Coin currency identity, and provisionally applies the same global pin to `/v2/account/wallet`; no keyed or endpoint-specific schema confirmation was performed. TKT-M21-01 re-fetched public `API:2/recipes` source on 2026-09-16: it confirms the 2022 recipe-schema ingredient `type`/`id`/`count` change needed for Currency/GuildUpgrade evidence, but does not establish the endpoint's latest pin; the recipe gateway therefore retains `2022-03-09T02:00:00.000Z` as provisional. | TKT-M13-03 / TKT-M19-04 / TKT-M21-01 | 2026-08-21 | — |
| VERIFY-006 | OPEN | Whether the GW2 API rate limit (per-IP token bucket; community-documented burst 300, refill 5/s) is applied per IP only or per IP+key, and the exact values. Values remain community estimates; M18 must keep safe configurable limits until evidence is sufficient. | wiki `API:Best_practices` fetched 2026-08-21 and 2026-08-22; `docs/rate-limiting/rate-limit-policy.md` §1/§5. TKT-M2-01's 2026-08-26 no-key probe observed `X-Rate-Limit-Limit: 600` on two HTTP 200 responses. TKT-M9-02's 2026-08-31 public `/v2/items` and `/v2.json` checks observed the same header on HTTP 200 responses. This does not prove limiter scope, burst/refill behavior, or a scheduler default. TKT-M18-02 (2026-09-08) retains the existing safe configurable request limits and performs no live probe. | TKT-M18-02 | 2026-08-21 | — |
...
| VERIFY-008 | OPEN | `/v2/account/recipes` scope is `account, unlocks` per the wiki infobox; confirm whether the `characters` scope also affects the unlocked recipe set, and confirm the exact server-side caching duration of `/v2/commerce/transactions` (wiki cites ~5 minutes). M13 owns transaction caching behavior; M21 owns recipe permission scope. | wiki `API:2/account/recipes`, `API:2/commerce/transactions` fetched 2026-08-21. TKT-M13-03 (2026-09-06) rechecked that the four transaction reads require `account, tradingpost`, history covers 90 days, and the documentation reports an approximately five-minute cache; the server's exact cache key, duration, and invalidation semantics remain unverified, so M13 adds no completed-response cache. TKT-M21-01 re-fetched the public `API:2/account/recipes` source on 2026-09-16: it confirms the `account, unlocks` infobox scope, a recipe-ID array response, and authorization-header use. TKT-M21-S05 reads every transaction page and treats a failed/incomplete relevant endpoint as provisional evidence. Buy-order cancellation absence remains provisional until two complete current-order/history captures span the bounded observation window; exact later completed-buy evidence reopens the plan for reconciliation. This is a conservative local policy, not a claim about ArenaNet cache coherence. It does not answer whether `characters` changes the unlock set or establish transaction cache semantics, so the uncertainty remains open. | TKT-M13-03 / TKT-M21-01 / TKT-M21-S05 | 2026-08-21 | — |
| VERIFY-010 | OPEN | Exact 429 response contract: whether the API sends `Retry-After` (and/or `X-RateLimit-*` headers) and a machine-readable body; the wiki documents neither, so the policy treats server-provided wait info as optional. | `docs/rate-limiting/rate-limit-policy.md` §1; wiki `API:2` fetched 2026-08-22. TKT-M18-02 (2026-09-08) reuses the gateway's tested optional-retry metadata handling and does not claim an upstream response contract. | TKT-M18-02 | 2026-08-22 | — |
| VERIFY-011 | OPEN | Empirical per-IP vs per-IP+key rate-limit behavior and the sustainable request rate for a single local read-only user; M18 may perform only the safe live check (≤2 read-only GETs, no burst) defined in the policy document with owner-approved key use where required. | `docs/rate-limiting/rate-limit-policy.md` §5. TKT-M18-02 (2026-09-08) performs no keyed live check and retains the existing conservative configurable defaults. | TKT-M18-02 | 2026-08-22 | — |
| VERIFY-012 | OPEN | A verified response-level discriminator, if any, for the community-observed transient "Invalid key" 403 condition. Until one exists, the gateway must treat all 401/403 responses as permanent and must not retry them. | `docs/rate-limiting/rate-limit-policy.md` §1/§3; TKT-M2-02 decision, 2026-08-27; TKT-M13-02 rechecked the public token-info/API-key documentation on 2026-09-05 and found no response-level discriminator. The gateway maps both to non-retrying invalid status. | TKT-M13-02 | 2026-08-27 | — |
| VERIFY-013 | OPEN | Exact Guild Wars 2 Trading Post fee contract for a sale: the canonical model uses a 5% listing fee plus 10% exchange fee, each with a 1-copper minimum and owner-approved independent round-up. Exact fractional-copper rounding remains externally unverified, so fee-derived profit remains explicitly modeled/provisional rather than authoritative external behavior. | TKT-M15-01 evidence review on 2026-09-07: ArenaNet's [Missing Gold](https://help.guildwars2.com/hc/en-us/articles/222384087-Missing-Gold) support article documents the separate rates, their timing, and the non-refundable listing fee; its linked official [Trading Post wiki page](https://wiki.guildwars2.com/wiki/Trading_Post) documents a 1c minimum for each fee. Neither source defines fractional-copper rounding. TKT-M15-01 therefore records completion path (b), keeps this item OPEN, and preserves the round-up behavior as provisional. | TKT-M15-01 | 2026-08-31 | — |
