# TKT-M21-S04 - Remove Superseded UI, Docs, and Code after Mes Signaux MVP
...
After the `Mes Signaux` MVP proves which existing capabilities are still needed, remove or archive superseded product surfaces and documentation without deleting useful backend financial/evidence engines.

## Requirements

- Audit React routes/pages/components made redundant by `Mes Signaux` and the new primary navigation.
...
- Remove or consolidate redundant Dashboard/Scanner/Personal Learning/Inventory presentation code only when required capabilities are already exposed through `Mes Signaux`, `Réglages`, diagnostics, or another retained surface.
...
- All retained user-facing UI copy touched by this ticket remains French.

## Acceptance criteria

- [ ] No duplicate primary user journey remains solely because of the pre-`Mes Signaux` UI structure.
