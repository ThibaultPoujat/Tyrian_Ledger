...
- Create src and tests structure from architecture.md.
- Create solution/project files with nullable and warnings-as-errors enabled where feasible.
- Create a minimal React frontend and backend that can be started locally.
- Add root README development commands.
- No business logic yet.

Ticket-specific non-goals:
- Implementing GW2 API access.


Delivery protocol (mandatory for every ticket):
- Create/use a dedicated branch named `ticket/<TKT-M1-01>-<short-kebab-title>`.
- Every commit for this ticket MUST start with `[TKT-M1-01]`.
- Before declaring the ticket complete, push the branch and create a GitHub pull request titled `[TKT-M1-01] Short title`.
