...
test('presents the execution-first Signal flow at 1920x1080 using only mocked local evidence', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Quatrième opportunité', level: 3 })).toBeVisible();
  await expect(page.getByRole('heading', { name: 'Cinquième opportunité', level: 3 })).toHaveCount(0);
