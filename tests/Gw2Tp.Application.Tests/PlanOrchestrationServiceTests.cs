...
public sealed class PlanOrchestrationServiceTests
{
    ...
    [Fact]
    public void Craft_confirms_when_a_verified_later_listing_consumed_its_output_before_inventory_refresh()
    {
        var craft = new PlanStep("craft", PlanStepAction.Craft, 100, "Insigne", 1, null, [], PlanStepState.Pending,
        ...
        var candidate = new PlanCandidate("craft-list", 1, "craft-list", PlanAttention.Active, [craft, list], [], Money.Zero, Money.Zero, 8_000, 0, 1, 1, true, []);
        var crafted = service.ReportStep(service.Start(candidate, Now, new Money(1_000), new Dictionary<string, long> { ["2:10"] = 1 }), 1, null, Now);
        var listed = service.ReportStep(crafted, 1, new Money(1_000), Now.AddSeconds(5));

        var reconciled = service.ReconcileWithVerifiedState(listed, new Money(990), new Dictionary<string, long> { ["2:10"] = 0, ["2:100"] = 0 }, Now.AddMinutes(1),
            [new PlanVerifiedEvidence("listing", PlanEvidenceKind.SellListing, 100, 1, new Money(1_000), Now.AddSeconds(5), Now.AddMinutes(1))], Now.AddMinutes(1), Complete(PlanEvidenceKind.SellListing));
        var effective = PlanOrchestrationService.ProjectEffectiveResources(new Money(990), new Dictionary<string, long> { ["2:10"] = 0, ["2:100"] = 0 }, reconciled.Events);

        Assert.All(reconciled.Events, execution => Assert.Equal(PlanShadowEventState.Confirmed, execution.State));
        Assert.Equal(0, effective.Quantities["2:100"]);
    }

    [Fact]
    public void Unchanged_fresh_inventory_after_a_reported_craft_requires_reconciliation()
    {
        var craft = new PlanStep("craft", PlanStepAction.Craft, 100, "Insigne", 1, null, [], PlanStepState.Pending,
            CraftEffects: [new(PlanResourceKind.Inventory, "10", -1, Money.Zero), new(PlanResourceKind.Inventory, "100", 1, Money.Zero)]);
        var candidate = new PlanCandidate("craft-stale", 1, "craft-stale", PlanAttention.Active, [craft], [], Money.Zero, Money.Zero, 8_000, 0, 1, 1, true, []);
        var reported = service.ReportStep(service.Start(candidate, Now, new Money(1_000), new Dictionary<string, long> { ["2:10"] = 1 }), 1, null, Now);
        var first = service.ReconcileWithVerifiedState(reported, new Money(1_000), new Dictionary<string, long> { ["2:10"] = 1 }, Now.AddMinutes(16), [], Now.AddMinutes(16));

        var reconciled = service.ReconcileWithVerifiedState(first, new Money(1_000), new Dictionary<string, long> { ["2:10"] = 1 }, Now.AddMinutes(32), [], Now.AddMinutes(32));
    ...
    [Fact]
    public void A_stabilized_cancel_after_one_mismatch_resumes_the_active_replacement_step()
    {
        var candidate = Candidate("cancel-resume", 100, 50, 1,
            steps:
            [
                new PlanStep("cancel", PlanStepAction.CancelBuyOrder, 42, "Objet", 1, new Money(100), [], PlanStepState.Pending, "order-7"),
                Step("replace", PlanStepAction.PlaceBuyOrder),
            ]);
        var reported = service.ReportStep(service.Start(candidate, Now), 1, new Money(100), Now);
        var visibleOrder = new PlanVerifiedEvidence("BuyOrder:7", PlanEvidenceKind.BuyOrder, 42, 1, new Money(100), Now.AddDays(-1), Now.AddMinutes(16), "order-7");

        var recheck = service.ReconcileWithVerifiedState(reported, new Money(1_000), new Dictionary<string, long>(), Now.AddMinutes(16),
            [visibleOrder], Now.AddMinutes(16), Complete(PlanEvidenceKind.BuyOrder, PlanEvidenceKind.CompletedBuy));
        var firstAbsence = service.ReconcileWithVerifiedState(recheck, new Money(1_000), new Dictionary<string, long>(), Now.AddMinutes(17),
            [], Now.AddMinutes(17), Complete(PlanEvidenceKind.BuyOrder, PlanEvidenceKind.CompletedBuy));
        var stabilized = service.ReconcileWithVerifiedState(firstAbsence, new Money(1_000), new Dictionary<string, long>(), Now.AddMinutes(32),
            [], Now.AddMinutes(32), Complete(PlanEvidenceKind.BuyOrder, PlanEvidenceKind.CompletedBuy));
        var replacement = service.ReportStep(stabilized, 1, new Money(100), Now.AddMinutes(33));

        Assert.Equal(PlanState.RecheckRequired, recheck.State);
        Assert.Equal(PlanState.InProgress, stabilized.State);
        Assert.Equal(PlanStepState.Current, stabilized.Steps[1].State);
        Assert.Equal(PlanStepAction.PlaceBuyOrder, replacement.Events[1].Action);
    }

    [Fact]
    public void Late_fill_reopens_a_terminal_cancelled_execution_after_the_next_step_is_declined()
    {
        var candidate = Candidate("cancel-terminal-late-fill", 100, 50, 1,
            steps:
            [
                new PlanStep("cancel", PlanStepAction.CancelBuyOrder, 42, "Objet", 1, new Money(100), [], PlanStepState.Pending, "order-7"),
                Step("replace", PlanStepAction.PlaceBuyOrder),
        ...
        var reported = service.ReportStep(service.Start(candidate, Now), 1, new Money(100), Now.AddSeconds(10));
        var cancelled = service.CancelUnperformedStep(reported);
        var firstAbsence = service.ReconcileWithVerifiedState(cancelled, new Money(1_000), new Dictionary<string, long>(), Now.AddMinutes(1),
            [], Now.AddMinutes(1), Complete(PlanEvidenceKind.BuyOrder, PlanEvidenceKind.CompletedBuy));
        var terminal = service.ReconcileWithVerifiedState(firstAbsence, new Money(1_000), new Dictionary<string, long>(), Now.AddMinutes(16),
            [], Now.AddMinutes(16), Complete(PlanEvidenceKind.BuyOrder, PlanEvidenceKind.CompletedBuy));

        var reopened = service.ReconcileWithVerifiedState(terminal, new Money(900), new Dictionary<string, long> { ["2:42"] = 1 }, Now.AddMinutes(17),
            [new PlanVerifiedEvidence("CompletedBuy:7", PlanEvidenceKind.CompletedBuy, 42, 1, new Money(100), Now.AddSeconds(5), Now.AddMinutes(17), "order-7")],
            Now.AddMinutes(17), Complete(PlanEvidenceKind.BuyOrder, PlanEvidenceKind.CompletedBuy));

        Assert.Equal(PlanState.Invalid, terminal.State);
        Assert.Equal(PlanState.ReconciliationRequired, reopened.State);
        Assert.Equal(PlanReconciliationState.Contradicted, reopened.ReconciliationState);
    }

    [Fact]
    public void Cancellation_reconciliation_expiry_is_anchored_when_the_cancellation_becomes_terminal()
    {
        var candidate = Candidate("cancel-retention-anchor", 100, 50, 1,
            steps:
            [
                new PlanStep("cancel", PlanStepAction.CancelBuyOrder, 42, "Objet", 1, new Money(100), [], PlanStepState.Pending, "order-7"),
                Step("replace", PlanStepAction.PlaceBuyOrder),
        ...
        var terminal = service.ReconcileWithVerifiedState(firstAbsence, new Money(1_000), new Dictionary<string, long>(), Now.AddMinutes(16),
            [], Now.AddMinutes(16), Complete(PlanEvidenceKind.BuyOrder, PlanEvidenceKind.CompletedBuy));
        var refreshed = service.ReconcileWithVerifiedState(terminal, new Money(1_000), new Dictionary<string, long>(), Now.AddMinutes(20),
            [], Now.AddMinutes(20), Complete(PlanEvidenceKind.BuyOrder, PlanEvidenceKind.CompletedBuy));

        var expiresAt = Assert.IsType<DateTimeOffset>(terminal.CancellationReconciliationExpiresAtUtc);
        Assert.Equal(expiresAt, refreshed.CancellationReconciliationExpiresAtUtc);
        Assert.True(refreshed.LastEvidenceCapturedAtUtc > terminal.LastEvidenceCapturedAtUtc);
        Assert.False(PlanOrchestrationService.IsCancellationReconciliationRetained(refreshed, expiresAt.AddTicks(1)));
    }

    [Theory]
    [InlineData(PlanStepAction.PlaceBuyOrder, PlanAttention.Active, PlanEvidenceKind.BuyOrder, PlanState.Waiting)]
    [InlineData(PlanStepAction.List, PlanAttention.Passive, PlanEvidenceKind.SellListing, PlanState.ExecutionComplete)]
    public void Terminal_recheck_recovery_uses_the_completed_action_not_plan_attention(
        PlanStepAction action, PlanAttention attention, PlanEvidenceKind evidenceKind, PlanState expectedState)
    {
        var candidate = Candidate($"terminal-{action}", 100, 50, 1, attention: attention, steps: [Step("terminal", action)]);
        var reported = service.ReportStep(service.Start(candidate, Now), 1, new Money(100), Now) with { State = PlanState.RecheckRequired };

        var reconciled = service.ReconcileWithVerifiedState(reported, new Money(900), new Dictionary<string, long> { ["2:42"] = action == PlanStepAction.PlaceBuyOrder ? 0 : 1 }, Now.AddMinutes(1),
            [new PlanVerifiedEvidence($"{evidenceKind}:1", evidenceKind, 42, 1, new Money(100), Now.AddSeconds(1), Now.AddMinutes(1))],
            Now.AddMinutes(1), Complete(evidenceKind));

        Assert.Equal(expectedState, reconciled.State);
        Assert.Equal(PlanStepState.Confirmed, reconciled.Steps[0].State);
    }
