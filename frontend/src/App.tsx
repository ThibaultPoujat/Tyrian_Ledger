...
function DashboardPanel({ dashboard, status }: { dashboard: Dashboard | null; status: 'loading' | 'error' | 'ready' }) {
  ...
  return <section className="dashboard-panel" aria-labelledby="performance-title">
    ...
    {dashboard.personalLearning !== null && <section className="dashboard-learning" aria-labelledby="learning-title">
      ...
      <p aria-live="polite"><strong>{personalLearningStatus(dashboard.personalLearning.status)}</strong> {dashboard.personalLearning.knownBasisSampleCount ?? 0} known-basis outcome{(dashboard.personalLearning.knownBasisSampleCount ?? 0) === 1 ? '' : 's'} retained; {dashboard.personalLearning.minimumKnownBasisSamples} required for strong evidence.</p>
      <div className="metric-grid">
        ...
        <section><h3>Observed completion windows</h3><strong>{dashboard.personalLearning.intervalCensoredCompletionCount}</strong><p>{dashboard.personalLearning.observedQuantityReductionCount} include an observed quantity reduction.</p></section>
