...
function PlanCard({ plan, post }: { plan: Plan; post: (path: string, body?: object) => void }) {
  ...
  return <li className="signal-card"><article aria-labelledby={`plan-${plan.id}`}><p className="action-badge">{orientation(plan.attention)}</p><h3 id={`plan-${plan.id}`}>{plan.state === 'ReconciliationRequired' ? 'Le plan doit être réconcilié' : 'Plan en cours'}</h3><p role="status">{planStateLabel(plan.state, plan.reconciliationState)}</p><ol>{plan.steps.map(step => <li key={step.id}><strong>{actionLabel(step.action)}</strong> — {step.itemName} · {step.quantity} unité{step.quantity === 1 ? '' : 's'} · {stepStateLabel(step.state)}</li>)}</ol>{current !== undefined && <div><button onClick={() => post(`/api/plans/${encodeURIComponent(plan.id)}/complete`, { quantity: current.quantity, unitPriceCopper: current.unitPrice?.copper ?? null })} type="button">Terminé</button><button onClick={() => post(`/api/plans/${encodeURIComponent(plan.id)}/complete`, { quantity: 0, unitPriceCopper: null, notPerformed: true })} type="button">Action non effectuée</button><details><summary>Exécution différente</summary><label>Quantité réellement exécutée <input min="1" onChange={event => setActualQuantity(event.target.value)} type="number" value={actualQuantity} /></label><label>Prix unitaire réel en cuivre <input min="0" onChange={event => setActualUnitPrice(event.target.value)} type="number" value={actualUnitPrice} /></label><button disabled={!/^[1-9]\d*$/.test(actualQuantity) || !/^\d+$/.test(actualUnitPrice)} onClick={() => post(`/api/plans/${encodeURIComponent(plan.id)}/complete`, { quantity: Number(actualQuantity), unitPriceCopper: actualUnitPrice })} type="button">Enregistrer l'exécution réelle</button></details></div>}{plan.hasUndoableEvent && <button onClick={() => post(`/api/plans/${encodeURIComponent(plan.id)}/undo`)} type="button">Annuler la dernière étape</button>}</article></li>;
}
...
function planStateLabel(state: string, reconciliation: string) { return state === 'ReconciliationRequired' || reconciliation === 'Contradicted' ? 'Le plan doit être réconcilié avant toute autre action.' : state === 'Waiting' ? 'Ordre placé : attente de confirmation du marché.' : 'Suivez uniquement l’action mise en avant.'; }
