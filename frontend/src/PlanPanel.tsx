...
type PlansResponse = { state: 'ready' | 'unavailable'; proposals: Proposal[]; plans: Plan[]; selection?: PlanSelection };
...
function PlanEmptyState({ selection }: { selection: PlanSelection | undefined }) {
  ...
  return <div className="signals-zero-state"><div><h3>Aucun plan à démarrer</h3><p>{planEmptyReason(selection?.reason)}</p>{selection && <p className="operational-status" role="status">{safetyGate > 0 ? `${selection.recommendationCandidates} signal${selection.recommendationCandidates === 1 ? '' : 'ux'} ont produit ${selection.generatedCandidates} candidat${selection.generatedCandidates === 1 ? '' : 's'}, mais ${safetyGate} candidat${safetyGate === 1 ? '' : 's'} ne ${safetyGate === 1 ? 'peut' : 'peuvent'} pas être exposé${safetyGate === 1 ? '' : 's'} tant que le dimensionnement du portefeuille n’est pas vérifié.` : `${selection.recommendationCandidates} signal${selection.recommendationCandidates === 1 ? '' : 'ux'} transformé${selection.recommendationCandidates === 1 ? '' : 's'} en candidat${selection.recommendationCandidates === 1 ? '' : 's'} · ${selection.rejectedHardConstraints} rejeté${selection.rejectedHardConstraints === 1 ? '' : 's'} par les contraintes strictes · ${selection.rejectedResourceConflicts} bloqué${selection.rejectedResourceConflicts === 1 ? '' : 's'} par les ressources ou réservations · ${selection.rejectedSelectionConstraints} écarté${selection.rejectedSelectionConstraints === 1 ? '' : 's'} à la sélection.`}</p>}</div></div>;
}
