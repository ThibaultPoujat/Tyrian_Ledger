...
export default function ScannerPanel() {
  ...
  return <section aria-labelledby="scanner-title" className="scanner-panel">
    ...
    {status === 'error' && <p role="alert">Scanner unavailable. Check the local host and try again.</p>}
    {status === 'ready' && result !== null && <>
      ...
      <section className="watchlist-summary" aria-label="Watchlist"><h3>Local watchlist</h3>
        {watchlistStatus === 'loading' ? <p role="status">Loading watchlist…</p> : watchlistStatus === 'error' ? <p role="alert">Watchlist unavailable. <button onClick={() => void loadWatchlist().catch(() => undefined)} type="button">Retry</button></p> : watching.length === 0 ? <p>No watched markets yet.</p> : <ul>{watching.map(itemId => <li key={itemId}>Item #{itemId} <button disabled={updating === itemId} onClick={() => toggle(itemId, true)} type="button">Remove</button></li>)}</ul>}
      </section>
