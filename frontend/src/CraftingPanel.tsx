...
export default function CraftingPanel() {
  ...
  if (data.state === 'NoOpportunities' || data.opportunities.length === 0) return <section className="signals-zero-state"><div><h2>Aucun parcours d’artisanat viable</h2><p>Les coûts, la liquidité ou les données disponibles ne justifient pas une action maintenant.</p><button className="refresh-signals" onClick={load} type="button">Actualiser</button></div></section>;
  return <section aria-labelledby="crafting-workspace-title" className="signals-feed">
    ...
    <ol className="signal-list" aria-label="Opportunités d’artisanat">{data.opportunities.map(opportunity => <li className="signal-card" key={opportunity.id}><article>
      ...
      {opportunity.isActionable && opportunity.planId ? <button onClick={() => start(opportunity.planId!)} type="button">Démarrer ce plan</button> : <p className="operational-status operational-status--warning">Non actionnable : {opportunity.exclusions.join(', ')}</p>}
...
function truncation(value: string) { return ({ RecipeLimit: 'nombre de recettes atteint.', DepthLimit: 'profondeur de recettes atteinte.', CandidateLimit: 'nombre de propositions atteint.', WorkLimit: 'budget de calcul atteint.' } as Record<string, string>)[value] ?? 'limite atteinte.'; }
